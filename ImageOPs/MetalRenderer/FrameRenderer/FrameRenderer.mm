...
- (instancetype)initWithDestSize:(CGSize)destSize andContext: (MetalContext *)context
{
    if ((self = [super init]))
    {
        _destTextureSize = destSize;
        _layer = nil;
        _metalContext=context;
        self.frameRendererEncoder = [[FrameRendererEncoder alloc] initWithContext: _metalContext];
        self.textureRendererEncoder = [[TextureRendererEncoder alloc] initWithContext: _metalContext];
...
- (void)buildTextures
{
    CGSize drawableSize = _destTextureSize;
    if (_layer) {
        drawableSize = _layer.drawableSize;
    }
